  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-[10px] text-muted-foreground">手指状态</span>
        <span
        ...
        <div className="flex-1 space-y-4">
          ...
          {direction === "NEUTRAL" && (
            <p className="text-sm text-muted-foreground">
              尚无手指 · 因子截至 {factorDate}
            </p>
          )}
          {direction !== "NEUTRAL" && activeSignal && (
            <p className="flex flex-wrap gap-x-1 text-sm text-muted-foreground">
              <span>{DIRECTION_LABEL[direction]}延续</span>
              <span>· {activeSignal.confirm_date ?? activeSignal.date} 起</span>
              <span>· 等待{reversalLabel}反转</span>
            </p>
          )}
        </div>
      </div>
