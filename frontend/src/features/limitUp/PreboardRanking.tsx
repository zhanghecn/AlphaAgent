        ...
        <table className="w-full min-w-[1020px] text-left text-xs">
          ...
          <tbody>
            {candidates.map((row, index) => (
              <tr key={row.vt_symbol} className="border-b last:border-b-0">
                ...
                <td className={cn("px-3 py-2 text-right font-medium tabular-nums", amountTone(row.change_pct))}>
                  {formatSignedPct(row.change_pct)}
                </td>
                <td className="px-3 py-2 text-right tabular-nums">{formatPct(row.distance_to_limit_pct)}</td>
                <td className={cn("px-3 py-2 text-right tabular-nums", amountTone(row.expected_d1_net_return_pct))}>
                  {formatSignedPct(row.expected_d1_net_return_pct)}
                </td>
                <td className={cn("px-3 py-2 text-right tabular-nums", probabilityTone(row.d1_win_probability))}>
                  {formatProbability(row.d1_win_probability)}
                </td>
                <td className="px-3 py-2 text-right font-medium tabular-nums">{formatProbability(row.touch_probability_3m)}</td>
                <td className="px-3 py-2 text-right font-medium tabular-nums">{formatProbability(row.eventual_touch_probability)}</td>
                <td className="px-3 py-2 text-right tabular-nums">{formatProbability(row.seal_probability_given_touch)}</td>
                <td className="px-3 py-2 text-muted-foreground">{sourceQualityLabel(row.source_quality)}</td>
