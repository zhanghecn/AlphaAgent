...
export const STAGE_META: Record<RuleStage, { label: string; tone: string }> = {
  ...
  preboard: { label: "板前买点", tone: "border-amber-500/50 bg-amber-500/10 text-amber-700 dark:text-amber-300" },
...
export function buildRuleFlow(guide: LimitUpStrategyGuide): RuleFlowNode[] {
  ...
  return [
    ...
    },
    {
      ...
      condition:
        "进入正式实时池后持续评估。只有原实时链已经给出买入结论，且同一 A/B/C 公共质量合同按此刻触板会通过，才显示板前买点；真实触板不是板前显示条件。",
      thresholds: [
        { label: "候选来源", value: "正式实时质量池" },
        { label: "质量与时点", value: "必须全部通过" },
        { label: "板前显示", value: "不要求真实触板" },
        { label: "快照", value: "≤ 20 秒" },
      ],
      dataNote: "当前价、A/C/B 层级、点时 D+1 质量估计、lane 验证和实时阻断状态。",
      dataGroupKeys: ["intraday", "prior"],
      failHint: "原实时链未形成买入结论、验证失败或快照过期时不显示板前买点。",
    },
    {
      id: "momentum",
      stage: "momentum",
