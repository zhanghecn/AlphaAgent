# -*- coding: utf-8 -*-
"""N型补涨打板(原趋势弱转强V4, 2026-08-30主人定调更名+按四组重组, 层编号退役):
只做 2板(前20日连板=2) 与 4+(前20日连板>=4), 各拆阴/阳 四组.
买点=断板后再启动的首个涨停板板上买(D0盘中触板买涨停价, 一字买不进排除) → 算打板;
形态内核=上波连板→断板洗盘坑→再启动首板的N型结构(坑三状态:无坑/坑内/已回顶)
→ 故名 N型补涨打板(2026-09-04由「U型」更名: N=整体结构, U蹲/V末反只是坑形地基分类).
...
任务:
  ...
  ③ 年月×组×地基 收益/胜率矩阵
输出: 容器 /app/w2s_v4_out/ (docker cp 回 N型补涨打板/)
"""
import os
import sys
...
#   曾收顶上规则追补(易德龙个案→全量验证): 2板系加「断板期收盘价从未站回顶上方」,
...
#   4+阴   孤立板穿插×全多头+++×洗盘坑存在×剔中坑回顶  27笔+5.56 胜率70% 分年全正
...
def pos_of(low_dd, pull):
    """坑三状态: 无坑(断板期最低收盘也在顶上方=一直新高没洗盘) / 已回顶(跌过坑又爬回顶上=起二波)
    / 坑内(坑进行时=买点区)."""
...
def tier_of(grp, base, seg_h=99, n_lim_mid=0, reb=0.0, gap=99, st="", dd=0.0, topped=True,
    ...
    topped=断板期曾收在顶上方(曾收顶上, 2板系要求 topped=False; 4+不用),
    ...
    if grp == "2板补涨阳":
        ...
        return False, ""
    if grp == "4板补涨阴":
        # 2026-08-30 两条追补: ①洗盘坑存在(无坑新高票-2.12毒) ②剔中坑8~15%×已回顶
...
def describe_row(r):
    ...
    if pos == "无坑":
        ...
        parts.append(f"坑{-dd * 100:.0f}%、已爬回顶上（突破起二波）")
    else:
...
def fail_reasons(r):
    ...
    if g == "2板补涨阴":
        if pos == "无坑":
            out.append("无坑新高票不进体系（没洗过盘）")
        elif pos == "已回顶":
            out.append("已爬回顶区（坑右墙已走完）")
        elif base == "DN":
...
def main():
    ...
    # 断板期曾收顶上判定用(曾收顶上规则, 2026-08-30追补): 锚=昨日前最后一次涨停日
    ...
    # 小波顶算的 → 大波深坑漏算被误判「无坑」(实测32笔夹层票+5.20). 重算为相对大波(≥4段)顶.
    ...
    # 真坑标注层用(struct_of): ≥2板段索引(同 build_base 口径, 排序保证 searchsorted 正确)
    ...
    # ── 真坑结构识别(标注层, 2026-09-01主人定调): 顶区=坑底日之前最高收盘(段内+断板期溢出,
    ...
    for name, c in GROUPS:
        ...
        if name.startswith("4板"):
            bigtop_fix(tg)
        # 真坑标注层(展示用, 判定仍用旧尺子): 位置/坑深/距顶/弹回/底确认/曾收顶上_真
        _st = [struct_of(s, p, name.startswith("4板")) for s, p in zip(tg["sid"], tg["pos"])]
        tg["位置"] = [s2["pos3"] if s2 else "无坑" for s2 in _st]
    ...
    # 位置列=真坑结构尺子(循环内已算); 出手判定(tier_of)仍用旧尺子字段
    ...
    for name, _ in GROUPS:
        ...
        for ym, sub in t[t["grp"] == name].groupby("ym"):
            ...
            # 层④12笔无坑夹层(+3.32胜率75%)=大波后高位横盘直接起小波的强势结构, 属U体系外例外
            nou = sub[(sub["位置"] == "无坑") & (sub["档位"] != "出手")]
            sub = sub[(sub["位置"] != "无坑") | (sub["档位"] == "出手")]
            if not len(sub):
                continue
            bad, good = sub[sub["bad"]], sub[~sub["bad"]]
            hit = sub[sub["档位"] == "出手"]
            lines = [f"# N型补涨打板 · {name} · {ym} · 好差票（坑坐标）", "",
                     ...
                     + (f" ｜ 另有无坑票 {len(nou)} 笔未列入（均未出手，板留均 {nou['r_bh'].mean() * 100:+.2f}%"
                    ...
                    + ["",
                     ...
                     "",
                     "**形态解读口径**",
                     "- 形态栏=真坑结构尺子（2026-09-01 起）：顶区=坑底日之前的最高收盘"
    ...
    # 年月×坑深矩阵md(坑口径: 坑深档列 + 出手列)
    mdir = os.path.join(OUT, "汇总")
    os.makedirs(mdir, exist_ok=True)
    DD_LABS = ["浅坑<8%", "中坑8~15", "深坑15~25", "超深>25%", "已回顶贴顶"]
    lines = ["# N型补涨打板四组 · 年月 × 坑深 · 收益/胜率矩阵（真坑结构口径）", "",
             "生成: w2s_v4_groups.py · 2026-09-01 · 板留%=板留断走均值(相对涨停价买入), "
             "单元格=n / 板留均 / 板留胜率。坑深=断板期最低收盘距顶区(坑底日前最高收盘, 插针不算)；"
             "已回顶贴顶=昨收距顶区>-4%。出手列=四组白名单命中(定稿尺子)。", ""]
    for name, _ in GROUPS:
        sub = t[(t["grp"] == name) & (t["位置"] != "无坑")].copy()   # 矩阵同库: 只留U形态
