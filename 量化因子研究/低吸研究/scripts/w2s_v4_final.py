# -*- coding: utf-8 -*-
"""U型补涨打板(原趋势弱转强V4)白名单定稿终版统计: ①四组覆盖率 ②每年/每月可交易日与触发密度 ③胜率收益.

白名单四组(2026-08-29主人定稿 → 2026-08-30重组: 层编号退役按组管理; 层③剔中坑已回顶毒格定稿):
  2板阴  U坑内蹲类地基(U型蹲/中位浅调/横盘平台/V末反/L趴底)×弹回≤16%×坑宽6-15×未收顶上
...
def main():
    ...
    # 白名单四组(2026-08-29融合定稿主人拍板; 08-30追补: ①2板系加「断板期未收顶上方」伪U排除
    #   ②4+阴剔「中坑8~15%×已回顶」毒格=半伤硬拉二次出货顶, 7笔-1.98, 34→27笔+4.01→+5.56逐年变厚):
    #   2板阴 U坑蹲类×坑宽6-15×未收顶上 | 2板阳 首阳×未收顶上 | 2板阳 纠缠态(两通道并行)
    ...
    L["2板阳·坑中纠缠态"] = tgs["2"][tgs["2"]["base"].isin(["U", "MID", "FLAT", "V", "LB"])
                                    & tgs["2"]["ma_st"].isin(["-++", "+--"])]
    nm3 = tgs["3"]["n_lim_mid"]
    brk3 = tgs["3"]["pull"] > -0.04                                   # 已爬回顶上(U突破)
