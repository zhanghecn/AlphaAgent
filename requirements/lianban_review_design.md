...
- ✅ **上证50/北证50 指数**：INDEX_SYMBOLS 补齐（c9dc0235），指数条 8 格全满
- ✅ **live 盘中指数**：`_fill_live_indices` 走实时行情填当日指数条（b1cc7bd7），
  08-14 盘中实测 mode=live 滚动更新（涨停 33/连板 10/最高 3 板）
- ✅ **连板天梯历史页** `/lianban/ladder`（55f822a9）：近 N 日梯队热力图 +
  晋级率矩阵（近 60 日 1进2=13.6%/4122 样本）+ 每日龙头轨迹
- ✅ **明日推演（同景统计）**（43131905）：与当日同情绪阶段+同上证年线位置的历史日
  → 次日上涨概率/均值/中位、次日阶段分布、同景日期清单；复盘页统计卡下方。
  为对齐 lianban 样本量级（96），情绪历史跨度 250 → 1250 交易日（重建 116s，
  得 820 个交易日——全市场日线覆盖上限），同景样本 6 → 85
- ⬜ 竞价雷达（四因子，竞价快照 07-13 起已累积）
- ⬜ 题材粒度归并（东财二级行业 → lianban 式行业+概念混合题材）
- ⬜ 北证50 历史回填（腾讯源仅当日，东财接口凌晨限流）；「📅N天」标记
