...
5. 正常全市场增量同步先从最近 10 个可靠交易日选择覆盖最宽的参考截面，再按股票代码
   精确比较目标日覆盖；参考日后新进入股票清单的代码也纳入检查。完整时不请求远端，
   有缺口时只请求缺失股票。局部结果保留给下一轮补齐，不再按 95% 数量阈值删除整日；
   仍不完整则记录 `DataCoverageIncomplete`，并禁止用它重建打板历史账本。显式强制/历史
   自举路径仍可执行全量刷新。
6. `schema.ensure_schema_once()` 在 API 或独立 worker 启动时自动补建兼容索引；覆盖审计
   使用 `ix_stock_daily_bars_date_symbol`，财报覆盖使用
   `ix_stock_financial_reports_source_date_symbol`。运行中的旧进程不会热加载这些索引，需在
   发布后的首次进程启动时生效。
...
可靠完整日必须满足至少 3000 个不同股票。共享库截止 `2026-07-29` 有 809 个可靠日，
范围 `2023-03-28..2026-07-29`；当日实存 5,531 个证券日线，日线覆盖审计为完整。这是
行情背景，不是正式推荐覆盖。当前 `limit-up-core-abc-v1` 的真实事件从 `2025-06-27`
开始，闭合推荐覆盖 `2025-07-10..2026-07-23`。
3% 雷达保存原始研究观测；3% 本身不生成候选或买点。实时链从中只公开“除真实触板外全部
正式条件已齐”的确定性板前候选，不显示未校准数值触板概率。滚动绩效仍以正式 A/B/C
触板买点和回测接口为准，板前观测不得混入正式推荐成绩。
...
- 情绪周期历史曲线以完整股票日线为输入，由 `sync_mainline_sentiment_history` 在盘后预计算
  并保存到 `mainline_sentiment_history`。页面请求只读取缓存和可用的当日快照投影，不在
  HTTP 请求中触发全市场日线窗口计算；当缓存缺失或日线仍在更新时返回构建状态并短暂重试。
...
- 21:30 重试日线、板块/个股资金、完整成员链路、免费证券状态、涨停池、同花顺证据、
  事件分钟、3% 雷达分钟和打板历史。没有板前决策冻结、结算或独立轮询任务。
...
生产 Compose 由独立 `alphaagent-data-sync-worker` 持有调度器，API 只处理请求；API
重启不会中断或重复启动调度。调度器自动执行这些计划；数据管理页“同步任务”可用“立即执行”
手动触发同一条服务端计划，API 为 `POST /api/data-sync/schedules/{schedule_id}/run`。手动
触发不改变数据来源，也不开放本地文件导入。

内存批次运行超过 2 小时时由看门狗终结。它只会标记该批次正在运行的任务、所属计划和
任务定义为失败，不会误伤独立任务；已经被终结的旧线程即使稍后返回，也不能把计划状态覆写为
成功。进程重启仍会将数据库残留的运行态标记为中断失败，由既有补偿计划接手。

季度财务任务按报告期批量读取东方财富 `RPT_LICO_FN_CPD`，不再每天逐股轮转 100 只或
并发请求三张报表。每个报告期按对应可靠日线截面的股票集合计算覆盖率，达到 95% 才可跳过；
未达标的当期在法定披露截止日前标记为“披露期内待补齐”，会每日重试但不阻断历史账本重建，
截止日后仍未达标才标记为覆盖不完整并阻断重建。首次自举最近 16 个季度，之后跳过已完整报告期
并每日刷新最新已结束季度；显式 `report_dates/symbols` 仅用于定向核验。批量源的 `SJLTZ` 是归母净利润同比，
...
源没有盘中时间，因此公告当日不用于盘中决策。仅实际变更的财报写入或旧同比失效后会清空同进程实时
质量缓存，避免 15:05 初步计划的“财报缺失”延续到 19:00 最终计划，重复响应不会改写 `updated_at` 或触发账本重建。当前表共 90,930 条、
...
- 数值板前概率、训练/评分状态机和冻结结算任务保持移除。`/api/limit-up/live` 独立输出
  确定性板前候选和正式买点：前者必须已通过 A/B/C 触板就绪、lane 验证、无其他阻断和
  快照新鲜度，后者仍只在真实触板或回封后通过完整质量门时输出。
- `limit_up_radar_observations` 继续保存 `change_pct >= 3` 的原始行情、概念、资金、历史
  点时字段、触板就绪和 lane 验证结论，用于离线研究和产品审计；数据库行本身不等于买点。
...
- 正式 A+B+C 首板实时板块门只读取当前快照：盘中行业触板扩散加当日行业资金，或新鲜完整的
...
- `limit_up_concept_strength_snapshots` 在 2026-07-15/16/17 分别保存 251/252/135 个
  ...
  概念分钟帧、信号时点行业触板数和当日行业资金，不能用日终或 D-1 数据补造正式盘中证据。
- 雷达 observation 已持久化 A+B+C 核心质量门、126 日辨识度、D-1 行业量能、D-1 五日
  收益和市场阶段，以及 `board_level`、有界的全部概念候选/成员数、概念成员快照日期与
  触发可用性。正式 C 每股每日取第一次封板/回封，并在信号时排除已经出现正式 A/B 的
  日期；每天只取第一笔补位。历史概念成员证据标记为
  `historical_proxy_pass_forward_unconfirmed`，实时缺少严格 D-1 成员时失败关闭。自然前向
  从 `2026-07-27` 开始，先前原始帧不迁移、不计入前向分母。
