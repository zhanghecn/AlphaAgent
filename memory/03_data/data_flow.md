...
- `limit-up-core-abc-v1` 正式账户不读取上述 14:30 研究价。它执行首板和二进三，
...
- 唯一活动板前合同为 `limit-up-preboard-decision-v2`。个股必须先通过正式公共 A/C/B
  质量准备，再由 `change_pct >= 3` 激活评分；质量失败股票可进入独立形态训练池，但不得
  被评分或推荐。已触板股票退出板前列表，继续走独立的正式触板质量链。
...
- 严格板前 C 只认行动后的第一条新报价且必须低于涨停价；一分钟历史代理使用下一分钟
  open，等于涨停价或缺少报价都算未成交。正式 A+B+C 触板基线使用信号后 20--60 秒保存
...
- 当前 v2 模型使用 144 个声明特征，其中 fit 实际使用 86 个、删除 58 个常量/不可用列；
  新增 D-1 情绪/行业/辨识度和盘中同行业 3%/5%/7%/9% 扩散。任何非有限可选值用显式
  missing 特征表达，不能静默删候选或用未来值回填。
- 金/银手指沿用 `features.market_snapshot_for_trade()` 的 D-1 已确认状态。概念强度、行业
  ...
  硬门，也不能用收盘数据回填。
- 最新 v2 validation 的三分钟/最终触板 Brier skill 为 `+0.0707/+0.1606`，PR-AUC 为
  `0.1743/0.3777`，Top20% lift 为 `3.8462/3.0769`。calibration 没有满足至少 10 笔、
  胜率 `>=60%` 且平均 D+1 为正的板前规则，因此仍是 `research_only`；模型指纹见
  `memory/06_backtests/limit_up_preboard_v2_quality_separation_20260727.md`。
