...
3% 雷达只保存原始研究观测，不计算概率、不生成候选或买点。滚动绩效以正式 A/B/C
回测接口为准，原始雷达数据不得混入正式推荐成绩。
...
- 板前概率、提前推荐、独立观察池、训练/评分状态机和冻结结算任务已全部移除。
  `/api/limit-up/live` 只在真实触板或回封后执行 `limit-up-core-abc-v2`，通过完整质量门后
  才输出正式推荐；涨停价买点只表示可尝试排队，不能证明真实成交。
- `limit_up_radar_observations` 继续保存 `change_pct >= 3` 的原始行情、概念、资金和历史
  点时字段，用于后续离线研究和数据审计。它不再产生概率、候选、动作或页面展示。
