...
- 金/银手指沿用 `features.market_snapshot_for_trade()` 的 D-1 已确认状态。概念强度、行业
  ...
  硬门，也不能用收盘数据回填。
- 最新冻结模型的概率排序通过：三分钟/最终触板 Brier skill 为 `+0.1409/+0.2252`，
  PR-AUC 为 `0.3290/0.4321`，机会 Top20% lift 为 `4.39/3.39`。但严格板前 C 首板账户
  只有 25 笔、52.00% 胜率、`+14.95%` 复利、`-9.17%` 回撤，低于 A 触板基线，状态为
