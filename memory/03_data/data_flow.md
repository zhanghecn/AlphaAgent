...
7. `limit_up_signal_snapshots` 按分钟保存实时审计，同一分钟内 15 秒扫描会更新该分钟最新 `captured_at`，不覆盖 `stock_events`；`limit_up_live_scan` 只在交易日 09:15-11:30、13:00-14:57 运行。独立的 `limit_up_live_trace_snapshots` 从主板涨幅 5% 开始逐次追加诊断快照，同一分钟不覆盖；保存概念预热、接近、触发、错过、封板、炸板和硬门证据，保留最近两个有诊断数据的交易日。行情全失败时追加 `scan_error`，诊断写入失败只进入正式快照的 `trace_cache_status/error` 和调度消息，不阻断正式推荐；19:00、21:30 任务补偿清理第三个交易日前的诊断行。独立的 `limit_up_concept_scan` 每 30 秒并发分页获取一次完整 A 股行情，最多 6 路；完整帧与 15 秒强势股/涨停池增量在内存中合并，所有主板非 ST 的 5% 雷达先完成概念和战法评估，再取动态 Top5、两仓和最多 6 条持续观察。第一次采到时已经封死且此前没有封板前帧的股票标记为错过，不事后补成买点。09:15-09:19 只允许 `observe / concept_warming / approaching_trigger`，09:20 后全部竞价硬门通过才允许 `trigger_ready`，执行许可仍固定为 `research_only`。实时服务自身还有第二层守卫：非有效交易阶段不请求外部行情也不落库；行情源日期错误、概念覆盖低于 90% 或完整概念帧超过 45 秒时禁止新买点。实时候选合并全市场行情、涨幅榜、涨停池和炸板池，并保存封板资金、封单/成交额、相邻快照封单保留率和变化率。AkShare 五类实时涨停池并行请求，vendored 六个涨停池 HTTP 入口统一使用 8 秒硬超时，单个公共源不得无限占住扫描线程。候选进入 PostgreSQL JSONB 前必须把 `prior_board.trade_date` 等 Python 日期转成 ISO 字符串；非空候选的 JSON 序列化属于持久化回归测试，不能再用空候选掩盖类型错误。只有间隔不超过 2 分钟的快照才累计连续封板时长和封单变化；封单较上一快照缩水超过 30% 时关闭可执行动作。缺 Tick/L2 时执行可信度固定标记为 `proxy_without_l2`。
...
产品组合不再使用逐笔动态退出。`lane=portfolio` 固定返回两仓综合首板、D+1 14:30 的 `scheduled_first_board_cash_replay`，缓存 6 小时；用户选择日期只裁剪回放范围，不重新选择仓位或时间。旧动态退出和四板位验证接口保留为内部研究，一进二消融也使用旧动态账户作同口径反例，不与当前产品收益混称。
