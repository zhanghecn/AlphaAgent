...
- 2026-06-22 Top80 后处理测试入口：`ALPHAAGENT_RUN_STRATEGY_POSTPROCESS_REPORT=1 uv run pytest tests/alphaagent/test_quant_strategy_acceptance.py::test_current_strategy_candidate_quality_postprocess_report -q -s`。它不改变真实策略，只从每天当前 Top80 池做信号日可见因子重排/坏日压缩，再独立模拟 Top20/Top10/Top5。报告现在同时输出 V2/V3/V4/V5/V6/V7、`data_quality` 和 `overall_without_price_discontinuity`。当前测试通道最好的是 `v2_cap8_overlay + mfe8_keep6_giveback5`：候选数 `845`，原始口径 `+0.2057%`、胜率 `33.14%`、平均 DD `-7.0588%`、最差 DD `-32.8267%`；剔除疑似断层后为 `+0.3884% / 33.33% / DD -6.9106% / worst -21.9124%`。这说明方向有效，但覆盖率下降且依赖数据质量审计，不能晋升真实策略。
...
- 2026-06-22 矩阵报告已共享同一批日线和原始评分缓存，并修复实验 variant 复用缓存时缺少市场上下文的问题。Top80 后处理和个股审计已新增本地候选快照缓存，`max_symbols=500` 命中后可在数秒内复用候选；矩阵报告和全市场 `5000` 只仍未完整快照化。全市场 `5000` 只曾在 `2:18` 后手动中断，瓶颈是 `dragon_pullback` 特征逐日逐票重算和市场上下文补算。后续如要做 full matrix，应扩展同版本候选快照、分段进度输出或复用评分快照。
