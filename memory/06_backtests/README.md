...
| --- | --- | --- | --- |
| `#120` | 当前日线 D+1 全历史组合回测页面基线 | 已由 `mainline_dragon_pullback / 0.1.1` 后台研究任务生成并通过 API/浏览器验证 | 2025-03-26 至 2026-06-16，收益约 `+44.45%`，最大回撤约 `-29.61%`，买入/卖出/持仓中 `199 / 189 / 10` |
...
- `2026-06-16_dragon_pullback_v0_1_1_refresh.md`: 当前 `mainline_dragon_pullback / 0.1.1` 刷新证据，含候选 `2026-06-16`、买卖记录 `#7`、组合回测 `#120`、`603629.SSE` 信号复核和页面验证。
...
- `/quant` 页面验证必须同时检查候选 tab 和回测 tab；不能只用 API 冒烟或前端 build 代替页面级验证。2026-06-16 已用 Playwright `chromiumSandbox=false` 复核：`/quant` 候选 tab 显示 `2026-06-16` 和 `0.1.1`，只有 1 个主操作 `刷新候选并回测`；回测 tab 显示 `#120` 和只读回测口径，报告接口较慢时也会先显示加载提示。`/stocks/603629.SSE` 优先展示组合回测 `#120` 的同一口径，K 线同时显示理论信号和实际买卖标记，包括 `2026-01-19 信号`、`2026-03-09 信号`、`2026-05-22 买`、`2026-06-09 卖`。
...
- 当前全历史 `#120` 收益约 `+44.45%`，最大回撤约 `-29.61%`；不能直接宣称策略稳定盈利。
