...
- Phase 1 v4 分钟级回测 A/B 裁决（同回测器同 universe，≥600 与 ≥300 两档）：
  **v4-B（白名单+深跌排除）两档唯一都赢 v3**（≥600 +5.81 vs +5.35；
  ...
  （ab 原始跑数与 5m 变体档已按 2026-08-02 中间产物清理删除，结论在文档内）。
...
- 扫板回测（真实排板成交模型，与分钟级同引擎同因子）：触板后开板按涨停价
  ...
  （窄口径跑数已清理，全量终版 `leader_sweep_backtest_full_baseline_20260802.json`）。
- 扫板质量研究（1164 触板事件，验证「炸板率高=选票问题」假说）：基线封板率
  ...
  `limit_up_leader_sweep_quality_20260801.json`（v2a 跑数已清理）。
