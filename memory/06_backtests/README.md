...
- 板前概率、提前推荐、独立观察池和模型冻结结算已从产品与研究代码中移除。3% 雷达只
  保存原始点时研究数据，不产生评分、候选、动作或正式成绩。
...
- 事件 v3 的 validation 完整质量池已有 21 个正式触板正例，但逐笔资金与宽行业扩散的
  股票日精确率和 D+1 均未达标，最高校准概率也仅 19.84%；当前没有可用公开概率阈值。
  在新的自然前向或更完整历史点时概念成员到达前，不重新缩放分数或生成板前买点。
...
- C2 联合模型已经证明单一 15 日 calibration 关系不能跨阶段复用，且 D+1 头在冻结
  validation 几乎没有超过 Base 的排序增量。不得在该 validation 上继续调阈值；等待严格
  概念达到 fit/calibration/validation 至少 20/10/10 日后，再预注册市场阶段稳定性和
  动态概念宽度/重叠惩罚，在新的未见窗口验证。
