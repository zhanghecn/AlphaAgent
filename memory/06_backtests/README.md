...
- `/limit-up` 是独立的沪深主板非 ST 打板研究工作台，不属于普通量化策略基线；创业板、科创板、北交所和特殊状态股票不进入候选。
- 当前 `limit-up-history-v9` 首板闭合 99 笔，D+1 开盘胜率 `60.61%`、平均净收益 `+0.7944%`、复利 `+105.71%`、最大回撤 `-15.97%`。规则冻结后的真实前向样本仍为 0，且缺逐日行业成员与 Tick/L2 成交证据，因此全部战法保持 `research_only`、`simulation_eligible=false`。
