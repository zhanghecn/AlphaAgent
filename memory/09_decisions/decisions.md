...
- 实时炸板率与 D-1 弱势修复状态继续作为风险信息和排序证据，但不再一票否决全市场
  买点。这两道盘中硬门不在历史正式回测中；主板封板不足 5 只以及个股结构、动能、
  资金、财务、半年基因、A/B/C 质量和验证门仍然生效。

### Formal historical and forward status

- 行情日线覆盖 `2023-03-28..2026-07-31` 的 811 个可靠交易日；正式历史账本仍止于
  `2026-07-30`，需再次显式重建后才会纳入 7 月 31 日。
- v2 全历史按因果顺序重放：141 笔闭合独立交割，`96/141=68.0851%`，平均净收益
  `+2.0608%`，独立信号复利 `+662.3119%`，最大回撤 `-21.0357%`，硬亏
  `10/141=7.0922%`。A/C/B 历史分层继续作为公共质量收缩先验，不再与当前聚合结果
  拼接成同一份成绩。
- 两仓成交 95 笔、`69/95=72.6316%`，平均净收益 `+2.3772%`、复利
  `+189.2273%`、回撤 `-8.8668%`。两仓跳过 46 条，其中 34 条因给后续 A 保留仓位。
...
- 最近闭合窗口 `2026-07-14..23` 全量只有 `6/13=46.1538%`，两仓 `2/4=50%`、
  ...
  达标替代近期退化事实。
- 7 月 24-30 日连续空仓来自历史首板“D-1 炸板率至少 35%”的分歧修复结构门；删除该门
  会把独立胜率降到 `59.69%`、复利降到 `+285.43%`，因此维持。它与已删除的实时
  D-1 全市场否决不是同一道条件。
- 7 月 31 日完整帧反事实得到 4 只同源板前候选，其中视觉中国、日盈电子随后触板，
  腾龙股份、锡业股份未触板。板前候选继续与真实触板后的正式推荐及回测分母分离。
- 当前状态固定为 `historical_proxy_pass_forward_unconfirmed`；历史 68.0851% 不是实盘
...
- `2026-07-30` A+B 次数门反向审计按交易日保留全部 D+1 `>=5%` 赢家，只移除
  `prior_limit_count_126` 的 `2-6` 条件。原门捕获每日最高赢家 `12/34`，说明漏选确实
  存在；但新增 161 笔全为 `>6`，胜率 `46.5839%`、均值 `-0.2325%`、硬亏率
  `22.9814%`。`7-9` 全样本 `38.8060%/-0.8987%`，`>=10` 在 2026-03..07 为
  `43.3333%/-0.6232%`，不能由 2025 局部结果改门。证据为
  `memory/06_backtests/limit_up_recognition_gate_reverse_daily_winner_20260730.md`；正式
  `v2` 和 C 的自然前向要求不变。
- 同日冻结 A+B 母池后，已枚举 42/63/126 交易日与 `1..10` 的 165 个连续区间；560/560
  个候选的现有 126 日计数和重算计数一致。42 日 `1-3`、63 日 `1-5` 和 126 日 `2-7` 仅是
  样本内描述性首位；126 日 `2-7` 的复利仅比 `2-6` 高 `1.55` 个百分点，却降低胜率和均值、
  增加回撤与硬亏率。正式预注册严格时序需要 498 个候选交易日，当前只有 252 个，任何变体
  都不得晋级；A+B 继续使用 `prior_limit_count_126` 的 `2-6`，等待新的自然前向样本。
...
- 3% 只是雷达采集下限，不是买点。板前买点只来自正式实时池：原实时链已形成买入结论、
  按当前时刻假设触板时同一 A/B/C 合同通过、lane 验证通过且快照新鲜。真实触板不再作为
  板前显示条件；不再生成通用观察名单，也不限制板前买点数量。触板后仍由原正式扫板链复核。
- 7 月 28-30 日删除额外市场门前的合同重放为 5 个板前买点、1 个后续触板；7 月 31 日
  删除两道额外市场否决后的合同重放为 4 个板前买点、2 个后续触板。两个执行口径不能
  合并外推稳定精确率；当前输出只证明“若随后触板则质量合同会通过”，不能证明“随后
  大概率触板”。详见 `memory/06_backtests/limit_up_preboard_actual_frame_audit_20260729.md`。
...
- 9:31-9:40 固定涨速/累计涨幅的“分钟级真实可执行回测”已删除：它先从事后确认的首板
  事件中选样本，再把入场放到首次触板前，样本池本身泄漏了“后来会触板”的结果，不能
  作为实时买入阈值、触板概率或收益证据。
...
- 首板龙头涨停价研究回测当前为 v2：封单/成交额比来自 D 日收盘封单，已从买入前选股
  因子删除；只保留 3 个 D-1 因子和触板时可见的流通市值。分批减仓按一次买入仓位统计
  胜负和双边费用；每日 TOP3 仍使用完整晨盘候选横截面，不等同于实时到达顺序。它是
  研究代理，不改变
  正式 A+B+C。实测单次运行超过两分钟，因此不放入统一 data-sync worker 常驻调度，
  只通过独立 research 服务或显式 CLI 运行。
- `2025-06-27..2026-07-31` 的 v2 显式回放为 663 个信号、419 个实际仓位，
  `175/419=41.7661%`、平均 `+1.5945%`、复利 `+538.4743%`、利润因子 `2.2385`、
  回撤 `-35.1982%`。旧 v1 把分批退出腿当独立交易形成的 `54.63%` 胜率已废弃；API
  拒绝展示版本不匹配的持久化结果，本地专用研究行已显式更新为 v2。
- 正式交易固定为 A+B+C v2：`96/141=68.0851%`。C 只能覆盖核心门的三个明确排除原因，
