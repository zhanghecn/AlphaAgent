...
- 唯一正式合同为 `limit-up-core-abc-v1`；历史、实时、调度和现金账本使用
  同一公开版本。当前代码、查询和页面不存在其他打板合同入口或兼容回退；数据库原始帧
  可以保留审计，但不会重标版本或进入当前成绩。
...
- A+B+C 已按因果顺序重放：143 笔全量独立闭合交割，`99/143=69.2308%`，平均
  净收益 `+2.1203%`，独立信号复利 `+762.4136%`，最大回撤 `-21.0357%`，硬亏
  `10/143=6.9930%`。A 为 `35/41=85.3659%`，C 为 `46/72=63.8889%`，B 为
  `18/30=60%`；全量口径必须报 A+B+C，不能只报 A。
- 143 笔分布在 108 个交易日，日均 1.3241 笔、单日最多 5 笔；“笔”是一只股票的
  一次独立买卖，同日可以有多笔。单仓实际成交 80 笔、复利 `+457.7327%`；两仓实际
  成交 96 笔、复利 `+226.6771%`，其中 34 条 B/C 因为后续 A 保留仓位而跳过。
- 当前状态固定为 `historical_proxy_pass_forward_unconfirmed`；历史 69.2308% 不是实盘
  胜率承诺。自然前向从 `2026-07-27` 起只累计当前合同的新信号；此前保存帧的反事实
  结果不进入前向分母。
- 核心门前 560 笔闭合事件有 179 个候选日，A+B 只保留 78 笔/50 日；排除组
  ...
  `46/72=63.8889%`。历史概念成员代理仍要求从 2026-07-27 起自然前向确认。
...
- 当日轨迹只读取当前 `limit-up-core-abc-v1` 帧；只有 `action=buy_now` 才计为“买点曾触发”。
...
- 正式交易固定为 A+B+C：`99/143=69.2308%`。C 只能覆盖核心门的三个明确排除原因，
...
- `limit-up-core-abc-v1` 的自然前向账本必须从新合同实际保存快照起算；最近
