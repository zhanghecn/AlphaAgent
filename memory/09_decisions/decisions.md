...
- 量化页组合回测列表按当前公开策略过滤；旧策略回测不能挤掉当前策略回测。
...
- 股票详情页如果最新组合回测已经有该股订单/成交，则 K 线只展示组合回测口径标记，不再叠加独立全局买卖记录的 `already_holding` 信号噪音；全局买卖记录只作为无组合执行记录时的兜底。
...
- `mainline_dragon_pullback / 0.1.0` 已成为普通历史研究唯一公开策略；旧 `mainline_leader_pullback / 0.1.1` 保留为内部备份/基线。当前全历史刷新见 `memory/06_backtests/2026-06-16_full_history_dragon_pullback_refresh.md` 和 `memory/06_backtests/README.md`：组合回测 `#118` 覆盖 `2025-03-26` 至 `2026-06-15`，收益约 `+66.78%`，最大回撤约 `-30.22%`，不能直接宣称策略稳定盈利。
