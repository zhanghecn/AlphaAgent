...
- `2026-07-28..31` 按当前 v2 合同因果逐帧重放得到 5 个板前买点，只有巨人网络后续触板，
  触板精确率 `1/5=20%`；四只未触板票最高只到 `5.821%-8.508%`。因此当前输出只证明
  “若随后触板则质量合同会通过”，不能证明“随后大概率触板”，也尚不具备可靠直接买入
  证据。详见 `memory/06_backtests/limit_up_preboard_actual_frame_audit_20260729.md`。
