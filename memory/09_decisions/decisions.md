...
- `requirements/alphaagent_pullback_low_suction_strategy_research.md` 已更新为当前回踩低吸优化研究结论：普通产品入口只公开 `mainline_dragon_pullback / 0.1.18`，低吸洗盘和龙回头在同一公开策略内作为内部 setup，不拆 UI，也不把低吸洗盘简单作为龙回头原始信号加分项。
- `mainline_dragon_pullback / 0.1.18` 是当前普通历史研究唯一公开策略代码；旧 `mainline_leader_pullback / 0.1.1` 和 `mainline_dragon_pullback / 0.1.0/0.1.1/0.1.3/0.1.6/0.1.8/0.1.9/0.1.11/0.1.14/0.1.15/0.1.16/0.1.17` 保留为内部备份/历史对照。
- `0.1.18` 保留低吸洗盘识别和严格执行线，但取消低吸固定保留名额。低吸只在“低吸很久 + 后续上升趋势”被确认后获得执行机会加分：低吸天数更长、MA5/MA10 不拉开、量能不过度放大、60 日涨幅不过度极端时才提高执行排序。该加分不改变原始 `total_score`，只影响执行机会分和原因解释。
- 低吸执行/换仓实验边界：`0.1.12/#162` 的 top20 外低吸补充执行池收益仅约 `+30.19%`，拒绝；`0.1.13/#163` 执行前 20 内低吸替换低效持仓可把回撤降到约 `-16.71%`，但收益降到约 `+46.88%`，拒绝作为当前基线；`0.1.15/#165` 收益约 `+55.41%`、最大回撤约 `-21.17%`，低吸覆盖改善但收益下降；`0.1.16/#167` 收益约 `+28.80%`，过度保守；`0.1.17/#168` 收益约 `+39.16%`、最大回撤约 `-27.63%`，低吸机会加分过宽。当前 `0.1.18/#169` 收益约 `+62.14%`、最大回撤约 `-19.22%`，作为当前代码证据。
...
- 当前完整全历史证据见 `memory/06_backtests/strategy_optimization_ledger.md` 和 `memory/06_backtests/README.md`：组合回测 `#169 / 0.1.18` 覆盖 `2025-03-26` 至 `2026-06-16`，收益约 `+62.14%`，最大回撤约 `-19.22%`；失败卖出实验 `#173 / 0.1.19` 收益约 `+54.40%`、最大回撤约 `-19.79%`；上一代码证据 `#165 / 0.1.15` 收益约 `+55.41%`，最大回撤约 `-21.17%`；性能参考 `#164 / 0.1.14` 收益约 `+63.96%`，最大回撤约 `-24.24%`，`#157 / 0.1.9` 收益约 `+64.25%`，最大回撤约 `-24.21%`；绩效对照 `#137 / 0.1.3` 收益约 `+63.36%`，最大回撤约 `-17.94%`。当前不能直接宣称 `0.1.18` 最终优化成功。
