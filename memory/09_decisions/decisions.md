...
- 89 日数据按 44 fit、15 calibration、30 validation 冻结；候选索引分层为
  `16,132 -> 15,970 -> 1,500 -> 1,044`，最终完整高质量路径包含 135 个后来触板
  和 909 个未触板股票日。翻转触板、封板、D+1 和正式入选标签不改变 membership。
- 数据指纹为
...
- 双概率资格已经通过。三分钟/最终触板头在 validation 的 Brier skill 为
  `+0.1409/+0.2252`，PR-AUC 为 `0.3290/0.4321`，机会 Top20% lift 为
  `4.39/3.39`；模型能在触板前产生有信息量的概率排序。
- 最终状态仍为 `historical_rejected`，因为严格 C 首板 25 笔只有 `52.00%` 胜率、
  `+14.95%` 复利、`-9.17%` 回撤，低于 A 首板的 22 笔、`68.18%`、`+28.90%`、
  `-4.66%`。37 个 C 动作最终触板率 `43.24%`，误报率 `56.76%`；概率排序有效不等于
...
- 2026-07-23 源码收口验收为打板后端 `794 passed`、data-sync `155 passed`、前端
  提交态 `120 passed`；compileall、前端生产构建、开发/部署 Compose 配置和差异检查均通过。
...
- 下一项有价值的独立研究不是继续在同一 validation 调阈值，而是预注册比较：纯触板概率
  排序、当前 D+1 优先排序，以及兼顾触板概率和占仓机会成本的单一排序目标；固定两仓和
  同一母池，逐项报告触板率、D+1 胜率、复利和回撤。
