...
- `GET /api/data-sync/tail-workflow`: 尾盘准备状态，包含最新完整日线、盘中快照、分钟线、候选日期、最近策略研究和 14:00/14:30/18:00 计划。
- `POST /api/data-sync/tail-workflow/prepare`: 手动执行 `tail_prepare_14h`，用于尾盘前快速准备关键数据。
...
- 默认定时计划为工作日 `14:00` 快同步关键尾盘数据、`14:30` 自动触发策略研究、`18:00` 补完整盘后数据。当前尾盘量化仍使用最新完整日线；今日盘中候选需要后续“历史日线 + 今日临时 Bar”引擎，不能污染 `stock_daily_bars`。
