...
- 2026-06-29 本地 `/mainline` 已改为“概念主线”口径并验证通过：`/api/mainline-replay/live?limit=80` 返回 `source=sector_fund_flows:concept` 且返回类型只有 `concept`，top10 示例为 `创新药、单抗概念、AI芯片、氦气概念、高压快充、全息技术、减肥药、创新医疗服务、IPO受益、CRO`；`/api/mainline-replay/snapshot?date=2026-06-26&limit=80` 返回只有 `concept`，top10 包含 `半导体概念` 和 `存储芯片`；`/api/mainline-replay/relation?sector_id=BK1137&date=2026-06-26&limit=20` 返回 `国产芯片、半导体概念、汽车芯片、高带宽内存、AI芯片、第三代半导体` 等关联概念。`上证50_ / AH股 / 茅指数 / 宁组合 / 风格 / 昨日涨停 / 近期新高` 等指数篮子或状态类伪概念已从概念主线过滤。验证：`uv run pytest tests/alphaagent/test_mainline_replay_api.py tests/alphaagent/test_mainline_replay_algo.py -q` 为 `29 passed, 1 warning`，`pnpm --dir frontend exec tsc -b` 通过，本地 `alphaagent-api` 和 `alphaagent-web` 已重建。
