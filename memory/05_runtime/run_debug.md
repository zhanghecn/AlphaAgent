...
- 2026-06-29 正式环境 06-29 概念排序优先级修正已部署验证：生产 `alphaagent-api` / `alphaagent-web` / `alphaagent-gateway` 镜像 label 均为 `org.opencontainers.image.version=v2.3.35`、revision `1a50f1db6ce707de6c1f43769cb3ab500cf0cb4c`。根因是 `rolling_board_count` 曾作为第一排序键，导致 `重组蛋白 BK1063` 因 2026-06-29 当天 `change_pct=9.2%` 以及近 7 交易日 3 次日涨幅前 10，被排在第 7；但它 20 点指数涨幅 `-0.087%`、连续热度只有 1 天，属于短线异动/反抽，不应压过 `存储芯片` 等连续主线。修复后排序优先级为状态、连续热度、20 点指数涨幅，再用滚动上榜次数、上榜均涨、热度/资金辅助。生产验证：`GET /api/system/version` 返回 `v2.3.35`；`snapshot?date=2026-06-29&limit=120` 与 `live?limit=120` 顺序一致，`重组蛋白 BK1063` 均为第 57，`index_change_pct=-0.087`、`rolling_board_count=3`、`continuation_days=1`；top20 不再被短线反抽概念占位。源码验证：`uv run pytest tests/alphaagent/test_mainline_replay_api.py tests/alphaagent/test_mainline_replay_algo.py tests/alphaagent/test_data_sync_schedule.py -q` 为 `93 passed, 1 warning`。
- 2026-06-29 正式环境 06-29 历史回放排名修复已部署验证：生产 `alphaagent-api` / `alphaagent-web` / `alphaagent-gateway` 镜像 label 均为 `org.opencontainers.image.version=v2.3.34`、revision `3afc509549a9f3a2f2154470c6b57a738a260c96`。根因是单日历史回放仍按旧 `sector_period_scores.heat_score` 排序并先截断 topN，随后才补 `index_points` / `rolling_board_count` / `continuation_days`，导致 2026-06-29 历史榜和实时概念指数榜口径不一致；同时 `历史新高`、`东方财富热股`、`题材股` 等状态/榜单篮子仍可能进入历史榜。修复后单日历史回放与实时榜一致：先拉足概念候选，补概念指数上下文，再按滚动上榜次数、连续热度、20 点指数涨幅、热度/资金辅助排序；状态/榜单篮子被过滤。生产验证：`GET /api/system/version` 返回 `v2.3.34`；`GET /api/mainline-replay/snapshot?date=2026-06-29&limit=30` top 为 `高带宽内存、中芯概念、先进封装、培育钻石、氦气概念`，`存储芯片 BK1137` 排第 12，`return_pct=30.13`、`rolling_board_count=2`、`continuation_days=8`，前 100 不含 `历史新高/东方财富热股/题材股/近期新高/趋势股`；同日 `live?limit=30` 顺序与历史回放一致。源码验证：`uv run pytest tests/alphaagent/test_mainline_replay_api.py tests/alphaagent/test_mainline_replay_algo.py tests/alphaagent/test_data_sync_schedule.py -q` 为 `92 passed, 1 warning`，`pnpm --dir frontend run build` 通过。
- 2026-06-29 正式环境 `/mainline` 概念指数和成分股修复已部署验证：生产 `alphaagent-api` / `alphaagent-web` / `alphaagent-gateway` 镜像 label 均为 `org.opencontainers.image.version=v2.3.33`、revision `81f7e94e34be9e3df7023204f9c572819e762959`。部署后生产库执行 `sync_sector_members` 与 `sync_stock_sector_memberships` 成功，`sync_job_runs` 记录 `rows_read/rows_written=86599`。经网关登录后 `GET /api/system/version` 返回 `v2.3.33`；`GET /api/mainline-replay/live?limit=80` 中 `存储芯片 BK1137` 排名第 13，`return_pct=27.0`、`rolling_board_count=2`、`continuation_status=maintained`、`stock_count=127`；`GET /api/mainline-replay/sector-stocks?sector_id=BK1137&date=2026-06-29&limit=10` 返回 `total=127`、`price_source=daily_bar`，默认 top 为 `华海清科 +20.00%、有研硅 +20.00%、阿石创 +19.06%`。Playwright 打开 `http://agu.yantiandao.com/mainline` 验证显示“成分股涨幅排行”，能看到 `存储芯片` 与涨幅靠前个股，实际请求包含 `sector_id=BK1137&sort_by=change_pct`。注意：用户口头给出的正式密码 `zhangxuan66` 登录失败，最终使用服务器 `.env` 中当前 `ADMIN_PASSWORD` 验证通过。
- 2026-06-29 本地 `/mainline` 成分股完整性和默认排序验证通过：`sync_sector_members` 修复外部源单页 100 条上限导致的提前停止，并把成员分页保护上限从 20 页提高到 100 页；随后本地容器执行 `run_job("sync_sector_members", {"page_size": 200})` 和 `run_job("sync_stock_sector_memberships")` 成功，最终 `rows_read/rows_written=86599`。重点核验：`存储芯片 BK1137` 为 `sectors.stock_count=127`、`sector_memberships=127`；`/api/mainline-replay/sector-stocks?sector_id=BK1137&date=2026-06-29&limit=10` 返回 `total=127`、`price_source=daily_bar`，默认 top 为 `华海清科 +20.00%、有研硅 +20.00%、阿石创 +19.06%`。`/api/mainline-replay/live?limit=80` 中 `存储芯片` 排名第 9，`return_pct=27.0`、`rolling_board_count=2`、`continuation_status=maintained`。Playwright 经 `http://localhost:8080/mainline` 登录验证显示“成分股涨幅排行”，选中 `存储芯片` 后请求 `sector_id=BK1137&sort_by=change_pct`。验证：`uv run python -m compileall alphaagent/server/api/mainline_replay.py alphaagent/server/services/data_sync.py`、`uv run pytest tests/alphaagent/test_mainline_replay_api.py tests/alphaagent/test_mainline_replay_algo.py tests/alphaagent/test_data_sync_schedule.py -q` 为 `91 passed, 1 warning`，`pnpm --dir frontend exec tsc -b` 和 `pnpm --dir frontend run build` 通过，`docker compose up -d --build alphaagent-api alphaagent-web` 后服务健康。
- 2026-06-29 本地 `/mainline` “概念指数”第一版验证通过：`/api/mainline-replay/live` 和 `snapshot` 排名项新增 `index_points`、`index_change_pct`、`continuation_status`、`continuation_days`、`activity_days_20`、`activity_ratio_20`、`previous_hot`；盘中实时追加 `temporary=true` 临时指数点，历史回放只读缓存。前端 `/mainline` 改为三栏概念指数工作台：左侧 `全部/维持/新起/断档` 筛选和微型指数线，中间概念指数曲线、连续热度和资金矩阵，右侧成分股资金流向与共振概念。验证：`uv run python -m compileall alphaagent/server/api/mainline_replay.py`、`uv run pytest tests/alphaagent/test_mainline_replay_api.py tests/alphaagent/test_mainline_replay_algo.py -q` 为 `35 passed, 1 warning`，`pnpm --dir frontend exec tsc -b` 和 `pnpm --dir frontend run build` 通过。`docker compose up -d --build alphaagent-api alphaagent-web alphaagent-gateway` 后本地后端直连 `live?limit=5` 返回前 3 个概念均含 20 个指数点和连续状态；Playwright 经网关登录 `/mainline` 确认显示“概念指数”、`全部/维持/新起/断档`、状态标签和 SVG 曲线，并请求 `live`、`sector-stocks`、`relation`、`timeline`。
- 2026-06-29 本地 `/mainline` 已改为“概念主线”口径并验证通过：`/api/mainline-replay/live?limit=80` 返回 `source=sector_fund_flows:concept`，top10 示例为 `创新药、单抗概念、AI芯片、氦气概念、高压快充、全息技术、减肥药、创新医疗服务、IPO受益、CRO`；`/api/mainline-replay/snapshot?date=2026-06-26&limit=80` top10 包含 `半导体概念` 和 `存储芯片`；`/api/mainline-replay/relation?sector_id=BK1137&date=2026-06-26&limit=20` 返回 `国产芯片、半导体概念、汽车芯片、高带宽内存、AI芯片、第三代半导体` 等关联概念。`上证50_ / AH股 / 茅指数 / 宁组合 / 风格 / 昨日涨停 / 近期新高` 等指数篮子或状态类伪概念已从概念主线过滤；主线 API 已拒绝外部 `sector_type` 参数，live/snapshot/relation payload 不再暴露 `sector_type`。验证：`uv run pytest tests/alphaagent/test_mainline_replay_api.py tests/alphaagent/test_mainline_replay_algo.py -q` 为 `33 passed, 1 warning`，`pnpm --dir frontend exec tsc -b` 与 `pnpm --dir frontend run build` 通过。
- 2026-06-29 正式环境概念主线彻底清理已部署验证：生产 `alphaagent-api` / `alphaagent-web` / `alphaagent-gateway` 镜像 label 均为 `org.opencontainers.image.version=v2.3.32`、revision `a914d954`。经网关登录后 `GET /api/system/version` 返回 `v2.3.32`；`GET /api/mainline-replay/live?limit=80` 返回 `source=sector_fund_flows:concept`，榜首示例 `创新药、高压快充、AI芯片、中芯概念`；`snapshot` 和 `relation` 返回 payload 均不含 `sector_type`；`GET /api/mainline-replay/snapshot?date=2026-06-26&sector_type=industry` 返回 `400 BAD_PARAMS`，确认旧行业兼容入口已关闭。
- 2026-06-29 正式环境个股详情盘中日期修复已部署验证：`v2.3.30` / revision `33e450ae`。`GET /api/stocks/301257.SZSE?date=2026-06-29` 返回 `price_source=intraday_snapshot`、`last_price=46.25`、`change_pct=7.11`；`GET /api/stocks/301257.SZSE/snapshot?date=2026-06-29` 返回 `technical_indicators.latest_close=46.25` 且 `temporary_bar=true`。正式页面 `/stocks/301257.SZSE?date=2026-06-29` smoke 确认显示 `46.25 / +7.11%` 和按今日临时 K 线计算的技术指标；页面请求不再触发旧 `/api/stocks/{vt_symbol}/indicators`。
- 2026-06-29 正式环境主线实时模式已部署验证：生产 `alphaagent-api` / `alphaagent-web` 镜像 label 均为 `org.opencontainers.image.version=v2.3.27`、revision `e88b6d26`。生产 `GET /api/mainline-replay/live?limit=5` 返回 `mode=live`、`trade_date=2026-06-29`、`base_daily_date=2026-06-26`、`source=sector_fund_flows`，榜首示例 `BK1216 医药生物`；`GET /api/mainline-replay/sector-stocks?sector_id=BK1216&date=2026-06-29&limit=3` 返回 `price_source=intraday_snapshot`。经网关登录正式 `/mainline` 的无沙箱 Playwright smoke 确认页面显示 `v2.3.27`、`今日实时 2026-06-29`、`源表：sector_fund_flows`、`盘中快照`，并实际请求 `/api/mainline-replay/live`、`sector-stocks?date=2026-06-29` 和 `relation?date=2026-06-26`。
- 2026-06-29 主线今日实时模式验证通过：`GET /api/mainline-replay/live?limit=5` 返回 `mode=live`、`trade_date=2026-06-29`、`base_daily_date=2026-06-26`，榜首示例 `BK1216 医药生物`，`source=sector_fund_flows`，分钟线更新时间 `2026-06-29T14:32:00`。`GET /api/mainline-replay/sector-stocks?sector_id=BK1216&date=2026-06-29&limit=3` 返回 `price_source=intraday_snapshot`；同接口查旧缺日线日期不会使用当前快照。验证：`uv run pytest tests/alphaagent/test_mainline_replay_algo.py tests/alphaagent/test_mainline_replay_api.py tests/alphaagent/test_data_sync_schedule.py tests/alphaagent/test_research_api.py -q` 为 `121 passed, 1 warning`；`npm run build` 通过；无沙箱 Playwright smoke 登录本地 `/mainline` 后确认默认显示“今日实时 2026-06-29”“源表：sector_fund_flows”“盘中快照”。
- 2026-06-29 主线关联概念 v2 验证通过：`uv run pytest tests/alphaagent/test_mainline_replay_algo.py tests/alphaagent/test_mainline_replay_api.py tests/alphaagent/test_data_sync_schedule.py tests/alphaagent/test_research_api.py -q` 为 `116 passed, 1 warning`；`npm run build` 通过；`docker compose up -d --build alphaagent-api alphaagent-web alphaagent-gateway` 后本地 `alphaagent-api/gateway` healthy。无沙箱 Playwright smoke 登录本地网关后确认 `/mainline` 渲染关联概念和成分股资金流向，关联概念按钮 `12` 个，点击第一个关联概念后仍保留资金流向/关联面板。
