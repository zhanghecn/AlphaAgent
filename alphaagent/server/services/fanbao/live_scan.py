"""断板反包打板盘中每分钟扫描:触板即买(无首刻窗)+ T字排板。

口径(反包规则.md v2 盘中规则;与高位接力最大差异=无首刻窗/无竞价门):
- 扫描窗口 09:30~15:00 全日;买入触发=现价首次 ≥ 涨停价,任意时刻都算
  (研究口径:反包日触板即买,未发现首刻过滤增益)
- 只对 actionable(命中方案点且非死格)的池票触发;雷达票只展示不写信号
- 一字开(开盘价≥涨停价)→ sealed_watch(T字观察):
    盘中打开(现价<涨停价)→ 按涨停价排板成交,entered(研究口径:T字可买,
    买价=涨停价;一字全天不开买不进);全天不开 → EOD 判 skipped_gap
- 竞价涨幅只记录展示,不设门(反包五点全部为 T-1 静态条件)
- 卖出由 EOD 定版(反包日炸板当日收盘走;封住→断板日收盘,15 日兜底)
...
from alphaagent.server.services.fanbao import contracts, repository

logger = logging.getLogger(__name__)
SHANGHAI = ZoneInfo("Asia/Shanghai")
_ADVISORY_LOCK_KEY = 726110
...
def run_live_scan_tick(now: datetime | None = None) -> dict[str, object]:
    ...
    try:
        ...
        logger.warning("fbb live scan failed: %s", exc, exc_info=True)
...
def _scan_once(today: date, pool: list[dict[str, object]], now: datetime) -> dict[str, object]:
    ...
    for entry in pool:
        if not bool(entry.get("actionable")):
            continue  # 雷达票(未命中/死格)只展示不触发,不写信号
        ...
        patch: dict[str, object] = {
            "name": entry.get("name"), "group6": entry.get("group6"),
...
def _save_run(trade_date: date, started: datetime, *, status: str,
    ...
    repository.save_scan_run(
        ...
        rules_version=contracts.FANBAO_RULES_VERSION, message=message, error=error,
    )
