"""断板反包打板(2/4/5+板断板1~3天后再涨停)API:实时推荐、回测报告、历史交割单、规则契约与同花顺条件。"""
...
def live_dates():
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "FBB_LIVE_DATES_UNAVAILABLE", "断板反包日期列表暂时不可用",
            {"reason": exc.__class__.__name__}))
...
def backtest():
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "FBB_BACKTEST_UNAVAILABLE", "断板反包回测报告暂时不可用",
            {"reason": exc.__class__.__name__}))
    if payload is None:
        return ok({"status": "unavailable", "message": "断板反包回测尚未运行",
                   "rebuild": rebuild})
...
def backtest_rebuild():
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "FBB_BACKTEST_REBUILD_UNAVAILABLE", "断板反包回测重算暂时不可用",
            {"reason": exc.__class__.__name__}))
    if result.get("already_running"):
        return JSONResponse(status_code=409, content=fail(
            "FBB_BACKTEST_RUNNING", "断板反包回测正在重算", result))
...
def backtest_status():
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "FBB_BACKTEST_STATUS_UNAVAILABLE", "断板反包回测状态暂时不可用",
            {"reason": exc.__class__.__name__}))
...
def ledger(trade_date: date | None = Query(default=None, alias="date"),
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "FBB_LEDGER_UNAVAILABLE", "断板反包交割单暂时不可用",
            {"reason": exc.__class__.__name__}))
