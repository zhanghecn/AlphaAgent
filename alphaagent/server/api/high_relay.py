"""高位接力打板(二接三/三接四)API:实时推荐、回测报告、历史交割单、规则契约与同花顺条件。"""
...
from alphaagent.server.services.high_relay import service

router = APIRouter(prefix="/high-relay", tags=["high-relay"])
...
def live(trade_date: date | None = Query(default=None, alias="date")):
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "HPR_LIVE_UNAVAILABLE", "高位接力实时推荐暂时不可用",
...
def live_dates():
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "HPR_LIVE_DATES_UNAVAILABLE", "高位接力日期列表暂时不可用",
            {"reason": exc.__class__.__name__}))
...
def backtest():
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "HPR_BACKTEST_UNAVAILABLE", "高位接力回测报告暂时不可用",
            {"reason": exc.__class__.__name__}))
    if payload is None:
        return ok({"status": "unavailable", "message": "高位接力回测尚未运行",
                   "rebuild": rebuild})
...
def backtest_rebuild():
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "HPR_BACKTEST_REBUILD_UNAVAILABLE", "高位接力回测重算暂时不可用",
            {"reason": exc.__class__.__name__}))
    if result.get("already_running"):
        return JSONResponse(status_code=409, content=fail(
            "HPR_BACKTEST_RUNNING", "高位接力回测正在重算", result))
...
def backtest_status():
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "HPR_BACKTEST_STATUS_UNAVAILABLE", "高位接力回测状态暂时不可用",
            {"reason": exc.__class__.__name__}))
...
def ledger(trade_date: date | None = Query(default=None, alias="date"),
    ...
    try:
        ...
        return JSONResponse(status_code=503, content=fail(
            "HPR_LEDGER_UNAVAILABLE", "高位接力交割单暂时不可用",
            {"reason": exc.__class__.__name__}))
