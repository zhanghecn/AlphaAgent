...
limit_up_preboard_point_actions = Table(
    ...
    Column("contract_version", String(80), nullable=False),
    Column("trade_date", Date, nullable=False),
    Column("daily_slot", Integer, nullable=False),
...
def _apply_compatible_schema_patches(engine) -> None:
    ...
    patches = (
        ...
        "WITH ranked AS (SELECT ctid, row_number() OVER (PARTITION BY model_fingerprint, trade_date ORDER BY captured_at, vt_symbol) AS slot FROM limit_up_preboard_point_actions WHERE daily_slot IS NULL) UPDATE limit_up_preboard_point_actions AS actions SET daily_slot = ranked.slot FROM ranked WHERE actions.ctid = ranked.ctid",
        "ALTER TABLE limit_up_preboard_point_actions ALTER COLUMN daily_slot SET NOT NULL",
